...
def run_test_draw_points_on_circle():
    ...
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(50, 100), 40)
...
def run_test_pizza():
    ...
    pizza(window, circle, 150, 'black', 2)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    title = 'POLYGON test 3: 15 segments with thin green lines'
    window = rg.RoseWindow(600, 600, title)
    circle = rg.Circle(rg.Point(300, 300), 250)
    circle.outline_thickness = 2
    polygon(window, circle, 15, 'green', 5)
    window.continue_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    title = ('FANCY POLYGON test 4:  300 blue lines circle,'
             + ' hops = 61.')
    window = rg.RoseWindow(600, 600, title)

    circle = rg.Circle(rg.Point(300, 300), 250)
    fancy_polygon(window, circle, 300, 61, 'blue', 1)
    window.close_on_mouse_click()
